...
func (o *OutTCPConn) ConnectToPeer(seconds uint32, remote, local string) {
	...
	o.logger.Info("Neighbor:", o.fsm.pConf.NeighborAddress, "FSM", o.fsm.id, "ConnectToPeer called",
		"OutTCPCOnn id", o.id)
	connTime := seconds - 3
	if connTime <= 0 {
		connTime = seconds
...
func (p *PeerConn) DecodeMessage(header *packet.BGPHeader, buf []byte) (*packet.BGPMessage, *packet.BGPMessageError,
	...
	if err != nil {
		...
		p.logger.Info("Neighbor:", p.fsm.pConf.NeighborAddress,
...
func (p *PeerConn) ReadPkt(doneCh chan bool, stopCh chan bool, exitCh chan bool) {
	...
	for {
		...
		case <-p.readCh:
			...
			if header.Type != packet.BGPMsgTypeKeepAlive {
				p.logger.Infof("Neighbor:%s FSM %d Received BGP packet %x", p.fsm.pConf.NeighborAddress,
					p.fsm.id, buf)
			}
